...
const PRESET_SETTINGS = [
  { key: 'g35', label: 'G35', path: './presets/g35.json', data: {
    "cameraMode": "third2",
    "isoCamD": 12,
    "thirdDist": 5,
    "thirdHeight": 3,
    ...
    "thirdAzimuth": 0.4025559215376209,
    "thirdLookAhead": 3.8,
    "thirdSmoothPos": 10,
    "thirdSmoothLook": 12,
    "thirdMouseLook": true,
    ...
    "thirdPitch": -0.26839999999999997,
...
function applyAllParams() {
  ...
  applyPlayerWeaponSettings();
  syncWeaponAmmoHud();
  p.enemyType = normalizeChoice(p.enemyType, ENEMY_TYPE_OPTIONS, 'rusher');
  p.enemyCount = Math.round(clampSetting(p.enemyCount, 0, 50, 0));
  p.enemyHealth = Math.round(clampSetting(p.enemyHealth, 1, 1000, 10));
  p.enemyInvincible = p.enemyInvincible === true;
  p.enemyBehavior = normalizeChoice(p.enemyBehavior, ENEMY_BEHAVIOR_OPTIONS, 'rush');
  p.enemyMoveSpeed = clampSetting(p.enemyMoveSpeed, 0, 12, 2.2);
  p.enemyDamage = Math.round(clampSetting(p.enemyDamage, 0, 250, 10));
  p.enemyAccuracy = Math.round(clampSetting(p.enemyAccuracy, 0, 100, 100));
  p.enemyPlacement = normalizeChoice(p.enemyPlacement, ENEMY_PLACEMENT_OPTIONS, 'random');
