...
import { clearZones, refreshZoneMarkers } from '../zones.js';

const sidebar = document.getElementById('sidebar');
const openSidebarSections = new Set();
...
const ICON_ALLIES = `<span class="sb-icon-mask sb-icon-allies" aria-hidden="true"></span>`;
const ICON_ZONES = `<span class="sb-icon-mask sb-icon-zones" aria-hidden="true"></span>`;
const ICON_BEHAVIOR = `<span class="sb-icon-mask sb-icon-behavior" aria-hidden="true"></span>`;
...
const DEFAULT_PRESET_KEY = 't3';
const PRESET_SETTINGS = [
  { key: DEFAULT_PRESET_KEY, label: 'T3', path: './presets/t3.json', data: {
      "cameraMode": "third2",
      "isoCamD": 12,
      "thirdDist": 5,
      "thirdHeight": 3,
      ...
      "thirdPitch": -0.10759999999999909,
      ...
      "editorCameraX": 5.288017108423675,
      "editorCameraY": 20.07974933033168,
      "editorCameraZ": 25.006965714126657,
      "editorYaw": 0,
      "editorPitch": -0.5323999999999981,
      "editorPlacedNpcs": [],
      ...
      "weaponPistolProjectileColor": "#ffff00",
      ...
      "weaponRifleProjectileColor": "#ffff00",
      ...
      "weaponShotgunProjectileLength": 2,
      ...
      "weaponRocketProjectileSpeed": 125,
      "weaponRocketProjectileSize": 0.3,
      "weaponRocketProjectileColor": "#ffff00",
      "weaponRocketProjectileBloom": true,
      "weaponRocketProjectileLength": 2,
      "weaponRocketProjectileBloomIntensity": 1.65,
      "weaponRocketProjectileBloomSize": 0.25,
      "weaponRocketRadius": 24,
      ...
      "playerSpawnInvincibleDuration": 3,
      "zoneRadius": 20,
      "zoneColor": "#0075ff",
      "zoneCapturable": true,
      "zoneRewardReinforcements": false,
      "zoneRewardHealth": false,
      "zoneRewardPoints": false,
      "zoneScore": 0,
      "zones": [
            {
                  "id": 1,
  ...
  { key: 'g58', label: 'G58', path: './presets/g58.json', data: {
      "cameraMode": "third2",
      "isoCamD": 12,
      "thirdDist": 5,
      "thirdHeight": 3,
      ...
      "third2BodyScreenY": 0.45,
      "third2MinEyeHeight": 0.15,
      "thirdAzimuth": 5.853000000000055,
      "thirdLookAhead": 3.8,
      "thirdSmoothPos": 10,
      "thirdSmoothLook": 12,
      "thirdMouseLook": true,
      ...
      "thirdMouseSensitivityX": 0.003,
      "thirdMouseSensitivityY": 0.0024,
      "thirdPitch": 0.0028000000000010616,
      "thirdOffsetMode": "parallel",
      "thirdOffsetX": 1.25,
      "thirdOffsetY": -0.25,
      "thirdOffsetZ": -0.25,
      ...
      "destructionDestructibleParticleDespawnTime": 1,
      "overallBloomIntensity": 1.8,
      "playerWeaponType": "rocketLauncher",
      "weaponInfiniteAmmo": false,
      "weaponPistolMagazineSize": 12,
      "weaponPistolTotalAmmo": 60,
      "weaponRifleMagazineSize": 30,
      ...
      "weaponShotgunProjectileSpeed": 400,
      "weaponShotgunProjectileSize": 0.05,
      "weaponShotgunProjectileColor": "#ffffff",
      "weaponShotgunProjectileBloom": true,
      "weaponShotgunProjectileLength": 2,
      "weaponShotgunProjectileBloomIntensity": 1,
      "weaponShotgunProjectileBloomSize": 1,
      ...
      "weaponRifleProjectileBloomColor": "#ff1100",
      "weaponRifleReticleOpacity": 0.5,
      "weaponShotgunProjectileBloomColor": "#ffff00",
      "weaponShotgunReticleOpacity": 0.5,
      "weaponSniperProjectileBloomColor": "#ff0000",
      "weaponSniperReticleOpacity": 0.5,
      "weaponGrenadeProjectileBloomColor": "#ff8844",
...
const ENEMY_BEHAVIOR_OPTIONS = [
  ...
  ['attackZone', 'Attack Zones / Enemies'],
  ['holdPosition', 'Hold Position'],
  ['roam', 'Roam'],
  ['defendPlayer', 'Defend Player'],
  ['rush', 'Rush'],
  ['orbit', 'Orbit'],
...
const ZONES_JSON_KEYS = [
  'zoneRadius',
  'zoneColor',
  'zoneCapturable',
  'zoneRewardReinforcements',
  'zoneRewardHealth',
  'zoneRewardPoints',
  'zoneScore',
  'zones',
...
function section(icon, title, buildFn) {
  ...
  const setSectionOpen = open => {
    ...
    hdr.querySelector('.arrow').textContent = open ? '▴' : '▾';
    if (open) openSidebarSections.add(key);
    else openSidebarSections.delete(key);
  };

  if (openSidebarSections.has(key)) {
    setSectionOpen(true);
  }
  ...
  hdr.addEventListener('click', () => {
    ...
    setSectionOpen(!body.classList.contains('open'));
  });
...
function buildPlayer(body) {
  ...
  body.appendChild(slider({ key: 'playerSpawnInvincibleDuration', label: 'Spawn Invincible', min: 0, max: 10, step: 0.1, dec: 1 }));
...
function setZonePlacementMode() {
  state.activePreset = 'custom';
  state.params.editorModeEnabled = true;
  state.params.editorPlacementTarget = 'zone';
  state.activeSlot = 1;
...
function buildZones(body) {
  body.appendChild(smallInfo('Editor mode zone placement: choose Zone as the placement target, aim at the grid floor, and left-click to place numbered non-overlapping zones. Right-click a zone to remove it.'));
  body.appendChild(slider({ key: 'zoneRadius', label: 'Radius', min: 0.5, max: 40, step: 0.5, dec: 1, onChange: applyAllParams }));
  body.appendChild(colorPicker('Color', 'zoneColor', applyAllParams));
  body.appendChild(toggle('Zones Can Be Captured', 'zoneCapturable', applyAllParams));
  body.appendChild(subhdr('Rewards'));
  body.appendChild(toggle('Reinforcements', 'zoneRewardReinforcements', applyAllParams));
  body.appendChild(toggle('Health', 'zoneRewardHealth', applyAllParams));
  body.appendChild(toggle('Points', 'zoneRewardPoints', applyAllParams));
  const stats = document.createElement('div');
  const zoneCount = Array.isArray(state.params.zones) ? state.params.zones.length : 0;
  stats.style.cssText = 'font-size:10px;color:var(--sb-muted);padding:4px 0 8px;line-height:1.5;';
  stats.textContent = `Zones: ${zoneCount} · Score: ${Math.round(Number(state.params.zoneScore) || 0)}`;
  body.appendChild(stats);
  const buttons = document.createElement('div');
  buttons.style.cssText = 'display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0 10px;';
  buttons.appendChild(btn('Place Zones', 'sb-btn-accent', () => {
    setZonePlacementMode();
    notify('Zone placement enabled ✓');
  }));
  buttons.appendChild(btn('Clear Zones', 'sb-btn-muted', () => {
    clearZones();
    applyAllParams();
    rebuildPanel();
  ...
  buildScopedJsonControls(body, 'Zones', ZONES_JSON_KEYS, 'zones.json');
}

function buildBehavior(body) {
  body.appendChild(smallInfo('Assign behaviors here or in the Enemies and Allies sections. Patrol Zone circles a placed zone, Attack Zones / Enemies prioritizes navigating to a zone, holds position once inside, and only uses combat targeting to defend or return fire, Hold Position keeps the unit stationary, Roam wanders until combat starts, and Defend Player follows the player.'));
  const syncEnemyBehavior = npcGroupSaveHandler('enemy', applyAllParams);
  const syncAllyBehavior = npcGroupSaveHandler('ally', applyAllParams);
  body.appendChild(select('Enemy Behavior', 'enemyBehavior', ENEMY_BEHAVIOR_OPTIONS, syncEnemyBehavior));
  body.appendChild(select('Ally Behavior', 'allyBehavior', ENEMY_BEHAVIOR_OPTIONS, syncAllyBehavior));
}
...
const SCENE_KEYS = [
  'placedObjects', 'editorPlacedNpcs', 'zones', 'zoneRadius', 'zoneColor', 'zoneCapturable', 'zoneRewardReinforcements', 'zoneRewardHealth', 'zoneRewardPoints', 'zoneScore',
...
function buildExportImport(container) {
  ...
  wrap.appendChild(btn('↩ Reset Defaults', 'sb-btn-muted', () => {
    const preset = PRESET_SETTINGS.find(item => item.key === DEFAULT_PRESET_KEY);
    applyParamObject(preset?.data || defaultParams);
    state.activePreset = DEFAULT_PRESET_KEY;
    applyAllParams();
    placePlayerAtActiveSpawn();
    rebuildPanel();
    notify('Reset ✓');
...
function applyAllParams() {
  ...
  p.editorPlacementTarget = ['asset', 'enemy', 'ally', 'playerSpawn', 'enemySpawn', 'allySpawn', 'zone'].includes(p.editorPlacementTarget) ? p.editorPlacementTarget : 'asset';
  ...
  p.playerSpawnInvincibleDuration = clampSetting(p.playerSpawnInvincibleDuration, 0, 10, 3);
  p.zoneRadius = clampSetting(p.zoneRadius, 0.5, 40, 4);
  p.zoneColor = hexSetting(p.zoneColor, '#0075ff');
  ...
  p.zoneScore = Math.max(0, Number(p.zoneScore) || 0);
  if (!Array.isArray(p.zones)) p.zones = [];
  p.zones = p.zones.map((zone, index) => ({
    id: Math.max(1, Math.round(Number(zone?.id) || index + 1)),
    x: Number.isFinite(Number(zone?.x)) ? Number(zone.x) : 0,
    y: Math.max(0, Number.isFinite(Number(zone?.y)) ? Number(zone.y) : 0),
    z: Number.isFinite(Number(zone?.z)) ? Number(zone.z) : 0,
    radius: clampSetting(zone?.radius, 0.5, 40, p.zoneRadius),
    color: hexSetting(zone?.color, p.zoneColor),
    capturable: zone?.capturable !== false,
    capturedBy: typeof zone?.capturedBy === 'string' ? zone.capturedBy : '',
    capturedTeam: zone?.capturedTeam === 'ally' || zone?.capturedTeam === 'enemy' ? zone.capturedTeam : '',
    ...
    rewards: {
      reinforcements: zone?.rewards?.reinforcements === true,
      health: zone?.rewards?.health === true,
      points: zone?.rewards?.points === true,
    },
  })).sort((a, b) => a.id - b.id);
...
function rebuildPanel() {
  ...
  const sectionDefs = [
    ...
    [ICON_ZONES, 'Zones', buildZones],
    [ICON_BEHAVIOR, 'Behavior', buildBehavior],
